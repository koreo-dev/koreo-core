...
class TestRegistry(unittest.TestCase):
    ...
    def test_roundtrip(self):
        ...
        functions = set(
            _name_generator("function") for _ in range(random.randint(5, 25))
        )
    ...
    def test_remove_usage(self):
        ...
        # Used throughout
        workflow_two_functions = common_functions.union(
            _name_generator("function") for _ in range(random.randint(2, 5))
        )

        # Used the original set
        workflow_one_original_functions = common_functions.union(
            _name_generator("function") for _ in range(random.randint(2, 5))
        )
        ...
        workflow_one_new_functions = common_functions.union(
            _name_generator("function") for _ in range(random.randint(2, 5))
        )
        registry.index_workflow_functions(
        ...
        for function in workflow_two_functions.difference(common_functions):
            workflows = registry.get_function_workflows(function=function)
            self.assertSequenceEqual([workflow_name_two], workflows)

        # Workflow One's unique _original_ functions no longer reference it.
        for function in workflow_one_original_functions.difference(common_functions):
            workflows = registry.get_function_workflows(function=function)
            self.assertNotIn(workflow_name_one, workflows)

        # Workflow One's unique _new_ functions should only reference it.
        for function in workflow_one_new_functions.difference(common_functions):
            workflows = registry.get_function_workflows(function=function)
            self.assertIn(workflow_name_one, workflows)
